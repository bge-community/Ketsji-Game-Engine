...
void EEVEE_create_game_shgroups(EEVEE_EffectsInfo *effects, EEVEE_TextureList *txl, DefaultTextureList *dtxl,
	DRWShadingGroup **bloomResolve, DRWShadingGroup **dofResolve)
{
	const bool use_highres = true;

	*bloomResolve = EEVEE_create_bloom_shgroup(effects, e_data.bloom_resolve_sh[use_highres], true);
	DRW_shgroup_uniform_float(*bloomResolve, "bloomIntensity", &effects->bloom_intensity, 1);

	*dofResolve = DRW_shgroup_create(e_data.dof_resolve_sh, NULL);
	DRW_shgroup_uniform_buffer(*dofResolve, "colorBuffer", &effects->source_buffer);
	DRW_shgroup_uniform_buffer(*dofResolve, "nearBuffer", &txl->dof_near_blur);
	DRW_shgroup_uniform_buffer(*dofResolve, "farBuffer", &txl->dof_far_blur);
	DRW_shgroup_uniform_buffer(*dofResolve, "depthBuffer", &dtxl->depth);
	DRW_shgroup_uniform_vec2(*dofResolve, "nearFar", effects->dof_near_far, 1);
	DRW_shgroup_uniform_vec3(*dofResolve, "dofParams", effects->dof_params, 1);
}
