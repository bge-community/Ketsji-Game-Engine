...
/** \file gameengine/GameLogic/SCA_CameraActuator.cpp
 *  \ingroup gamelogic
 */
...
void SCA_CameraActuator::Relink(std::map<void *, void *> &obj_map)
{
...
static void Kx_VecUpMat3(float vec[3], float mat[3][3], short axis)
{
	...
	mat[coz][1] = vec[1];
	mat[coz][2] = vec[2];
	if (normalize_v3((float *)mat[coz]) == 0.0f) {
	...
	if (normalize_v3((float *)mat[coy]) == 0.0f) {
...
bool SCA_CameraActuator::Update(double curtime, bool frame)
{
	//wondering... is it really necessary/desirable to suppress negative events here?
	...
	// CONSTRAINT 2: Can camera see actor? (not implemented)
	...
	// CONSTRAINT 6: Again: fixed height relative to floor below actor (not implemented)
	// CONSTRAINT 7: Track to floor below actor
	// CONSTRAINT 8: Look a little bit left or right,
	//               depending on how the character is looking (horizontal x) (not implemented)
...
PyAttributeDef SCA_CameraActuator::Attributes[] = {
	...
	KX_PYATTRIBUTE_FLOAT_RW("damping", 0.0f, 10.0f, SCA_CameraActuator, m_damping),
	KX_PYATTRIBUTE_NULL
};

PyObject *SCA_CameraActuator::pyattr_get_object(void *self_v, const KX_PYATTRIBUTE_DEF *attrdef)
{
...
int SCA_CameraActuator::pyattr_set_object(void *self_v, const KX_PYATTRIBUTE_DEF *attrdef, PyObject *value)
{
	SCA_CameraActuator *self = static_cast<SCA_CameraActuator *>(self_v);
